...
def _phase1_paths() -> tuple[Path, dict]:
    ...
    if not pointer.exists(): raise HTTPException(status_code=503, detail="Price dataset is unavailable.")
    try:
    ...
    if not dataset.is_dir(): raise HTTPException(status_code=503, detail="Published price dataset is missing.")
    return dataset, metadata

@app.get("/api/v1/health")
def health() -> dict:
    try: store.get(); ready = True; detail = None
    except (MetricsCacheUnavailable, MetricsCacheStale) as exc: ready = False; detail = str(exc)
...
def stock(symbol: str) -> dict:
    ...
    if rows.empty: raise HTTPException(status_code=404, detail=f"Stock {symbol} is not in the current eligible universe.")
...
def stock_chart(symbol: str, days: int = Query(252, ge=20, le=2520)) -> dict:
    ...
    except (OSError, ValueError, ImportError, KeyError) as exc: raise HTTPException(status_code=404, detail=f"Chart data unavailable for {symbol}.") from exc
