...
const demo: Row[] = [
  {Rank:1,Symbol:"CDSL", "Company Name":"CDSL Ltd",Industry:"Capital Markets",Index:"NIFTY MIDCAP 150", "Momentum Score":94.2,CMP:1572.4,"3M Return":28.4,"6M Return":52.1,"3M Sharpe":2.14,"6M Sharpe":1.89,"R² 1Y":.86,"% From 52W High":-4.2,"% EMA 50":7.8,"% EMA 200":24.3,"Volume Ratio":2.35},
  {Rank:2,Symbol:"IRFC", "Company Name":"Indian Railway Fin Corp",Industry:"Finance",Index:"NIFTY 50", "Momentum Score":92.8,CMP:172.35,"3M Return":24.6,"6M Return":46.8,"3M Sharpe":1.98,"6M Sharpe":1.76,"R² 1Y":.82,"% From 52W High":-6.1,"% EMA 50":6.2,"% EMA 200":21.7,"Volume Ratio":1.92},
  {Rank:3,Symbol:"RECLTD", "Company Name":"REC Ltd",Industry:"Finance",Index:"NIFTY NEXT 50", "Momentum Score":91.6,CMP:604.8,"3M Return":22.1,"6M Return":41.3,"3M Sharpe":1.87,"6M Sharpe":1.65,"R² 1Y":.79,"% From 52W High":-5.3,"% EMA 50":5.1,"% EMA 200":19.8,"Volume Ratio":1.78},
  {Rank:4,Symbol:"MAHLOG", "Company Name":"Mahindra Logistics",Industry:"Logistics",Index:"NIFTY SMALLCAP 250", "Momentum Score":90.3,CMP:428.65,"3M Return":19.8,"6M Return":38.7,"3M Sharpe":1.75,"6M Sharpe":1.52,"R² 1Y":.76,"% From 52W High":-7.2,"% EMA 50":4.3,"% EMA 200":17.6,"Volume Ratio":2.11},
  {Rank:5,Symbol:"PNB", "Company Name":"Punjab National Bank",Industry:"Banking",Index:"NIFTY 50", "Momentum Score":89.7,CMP:112.25,"3M Return":18.7,"6M Return":36.2,"3M Sharpe":1.62,"6M Sharpe":1.41,"R² 1Y":.72,"% From 52W High":-8.9,"% EMA 50":3.8,"% EMA 200":15.9,"Volume Ratio":1.65},
  {Rank:6,Symbol:"HUDCO", "Company Name":"Housing & Urban Dev Corp",Industry:"Finance",Index:"NIFTY MICROCAP 250", "Momentum Score":88.9,CMP:215.6,"3M Return":21.3,"6M Return":34.8,"3M Sharpe":1.68,"6M Sharpe":1.38,"R² 1Y":.74,"% From 52W High":-9.1,"% EMA 50":2.9,"% EMA 200":14.2,"Volume Ratio":2.41}
];
    ...
    <main>
      ...
      <section className="page">
        ...
        <div className="tablecard"><div className="tablehead"><div><b>Ranked Results</b><span> ({total})</span>{loading&&<span className="loading"> · updating…</span>}</div><div className="tools"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search stock…"/><button className="selected">▤ Table</button><button>⇩ Export</button></div></div>
          <div className="tablewrap"><table><thead><tr>{columns.map(c=><th key={c} onClick={()=>doSort(c)}>{c} <span>↕</span></th>)}</tr></thead><tbody>{shown.map((r,i)=><tr key={String(r.Symbol)}><td>{r.Rank ?? i+1}</td><td className="symbol">{r.Symbol}</td><td>{r["Company Name"]}</td><td>{r.Industry}</td><td>{r.Index}</td><td><mark>{fmt(r["Momentum Score"])}</mark></td><td>₹{fmt(r.CMP)}</td><td className="green">{pct(r["3M Return"])}</td><td className="green">{pct(r["6M Return"])}</td><td>{fmt(r["3M Sharpe"])}</td><td>{fmt(r["6M Sharpe"])}</td><td>{fmt(r["R² 1Y"])}</td><td className="red">{pct(r["% From 52W High"])}</td><td className="green">{pct(r["% EMA 50"])}</td><td className="green">{pct(r["% EMA 200"])}</td><td>{fmt(r["Volume Ratio"])}x</td></tr>)}</tbody></table></div>
