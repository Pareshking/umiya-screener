...
export default function StockPage() {
  ...
  useEffect(() => {
    if (!symbol) return;
    Promise.all([
      fetch(`${API}/api/v1/stocks/${encodeURIComponent(symbol)}`, {cache:"no-store"}).then(async r => { if (!r.ok) throw new Error((await r.json().catch(()=>({}))).detail || "Stock unavailable"); return r.json(); }),
      fetch(`${API}/api/v1/stocks/${encodeURIComponent(symbol)}/chart?days=252`, {cache:"no-store"}).then(async r => { if (!r.ok) throw new Error((await r.json().catch(()=>({}))).detail || "Chart unavailable"); return r.json(); })
    ]).then(([s,c]) => { setStock(s); setChart(c.rows || []); }).catch(e => setError(e instanceof Error ? e.message : "Unable to load stock.") );
  }, [symbol]);
  ...
  if (error) return <main className="detailpage"><button className="back" onClick={()=>router.push("/")}>← Screener</button><div className="detailerror"><b>{error}</b><span>Return to the screener and try another symbol.</span></div></main>;
  ...
  return <main className="detailpage">
    ...
    <section className="detailgrid">
      <div className="detailcard chartcard"><div className="cardtitle"><b>Adjusted Close</b><span>1Y · As of {stock["Market As Of"] || "—"}</span></div>{path ? <div className="chart"><svg viewBox="0 0 100 100" preserveAspectRatio="none"><polyline points={path} fill="none" stroke="currentColor" strokeWidth="1.2" vectorEffect="non-scaling-stroke" /></svg></div> : <div className="chartempty">No chart data</div>}<div className="chartlabels"><span>{chart[0]?.date?.slice(0,10) || "—"}</span><span>{chart.at(-1)?.date?.slice(0,10) || "—"}</span></div></div>
